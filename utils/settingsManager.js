const { softGreen, Orange } = require('../assets/global/colors.json');
	...
	switch (selectedState) {
		...
		case 'on': {
			if (readData[setting].state === true) {
				return message.channel.send({ embeds: [new MessageEmbed().setColor(Orange).setDescription(`${readData[setting].humanizedString} setting is already \`enabled\``)] });
			} else {
		...
		case 'off': {
			if (readData[setting].state === false) {
				return message.channel.send({ embeds: [new MessageEmbed().setColor(Orange).setDescription(`${readData[setting].humanizedString} is already \`disabled\``)] });
			} else {
